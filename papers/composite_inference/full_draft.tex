...
%Such common knowledge is rarely available in real-world apps.

% merely trace boundaries between classes in the data representation space and therefore lack built-in information to interpret their predictions. 
...
%use of low-dimensional generative models.
...
% For instance, we can get insight such as: ``this patient is more likely to be AD than healthy because his hippocampus is severly atrophied even though his memory tests are quite normal.''

%So, if a feature yields a significant test but has low weight, it may have little influence on the comparison. 

%How about overall feature influence? Well, if we want to really interpret the prediction, we shall go through each target comparison that is deemed important for the decision. For instance, the system says AD more likely than normal, but is AD more likely than MCI? To answer this, we need to apply the procedure to AD vs MCI, and maybe other features will pop up.
...
%See if the automated prediction makes sense. If not, stop, otherwise consider what other class the interpretation could suggest and ask for comparison. Repeat the process until all other possibilities are subjectively ruled out or the user decides to reject the prediction. Think about this as a human AI dialog. 

%This is a recipe for {\em local} interpretation (why did the system make a particular prediction), but what about {\em global} interpretation (how does the system make predictions). The thing is, we may not care about global interpretation. The only thing we need to know is that the system uses a bunch of features that all potentially contribute to the predictions. We could investigate which features tend to contribute most notably and which ones do not {\em on the average} but this would only be interesting design purposes, i.e. if we are prepared to use another representation. 

%We may represent feature influence by a $K\times K$ matrix with general element $\lambda_i r_i(x_k,x_\ell)$. 

%This recipe addresses {\em local} interpretation (why did the system make a particular prediction) as opposed to {\em global} interpretation (how does the system make predictions in general). 
