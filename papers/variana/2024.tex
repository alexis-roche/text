...
\item 2014--16: realization that VS is useful combined with Expectation Propagation.
...
\item 2023: key insight: VS leverages more information from a sample than stochastic gradient descent on KL~divergence.
\end{itemize}
...
The gradient and Hessian of the KL~proxy are given by:
$$
\nabla \tilde{D} = \frac{1}{n}\sum_i [g_\theta(x_i)-f(x_i)]\phi(x_i),
\qquad
\nabla \nabla^T \tilde{D} = \frac{1}{n}\sum_i g_\theta(x_i)\phi(x_i)\phi(x_i)^\top
,
$$
which can be interpreted as IS estimates.
...
The gradient and Hessian of this alternative KL~proxy are seen to be:
$$
\nabla \tilde{D}_{IS} = \int \pi g_\theta \phi - \frac{1}{n}\sum_i f(x_i)\phi(x_i),
\qquad
\nabla \nabla^T \tilde{D}_{IS} = \int \pi g_\theta \phi \phi^\top
.
$$
...
We have seen that the VS moment estimate converges in distribution to the true moment if the following matrix is finite:
$$
\Sigma = \int \pi (f - g_\star)^2 \phi \phi^\top
.
$$

Can we give simple conditions for it to be the case? 

Let us focus for now on the first diagonal element:
$$
\sigma_0^2 = \int \pi (f - g_\star)^2.
$$

We note that $(f-g_\star)^2 = f^2 - 2fg_\star + g_\star^2\leq f^2 + g_\star^2$. So, if both $\int \pi f^2$ and $\int \pi g_\star^2$ exist, then $\sigma_0^2$ is finite. Both conditions depend on the distribution tails of $f$ and $g_\star$, respectively.

In practice, we might be able to ascertain that the target $f$ is bounded, so that $\|f\|_\infty$ is finite and $\int \pi f^2$ is therefore finite.

What about $g_\star$? Let's go back to the KL~minimization. The variances of $\pi f$ and $\pi g_\star$ are the same owing to the moment matching property of KL~minimization, and so are the precisions:
$$
\prec(\pi g_\star) = \prec(\pi f).
$$

Now, both $\pi$ and $g_\star$ are log-quadratic functions so the precisions satisfy some algebra rules:
$$
\prec(\pi g_\star^2)
= 2 \prec(\pi g_\star) - \prec(\pi)
= 2 \prec(\pi f) - \prec(\pi).
$$

Clearly, $\int \pi g_\star^2$ is finite iff $\prec(\pi g_\star^2)$ is positive (not necessarily positive definite), which brings the condition:
$$
\prec(\pi f) \succeq \frac{1}{2}\prec(\pi)
.
$$

In other words, the ``posterior'' variance should not be more than twice as large as the ``prior'' variance:
$$
\var(\pi f) \preceq 2 \var(\pi)
.
$$

It feels like a pretty mild condition, but it's not implied by the condition that $f$ is bounded.


\subsection{Damning counter-example}
...
In this case, the optimal fit $g_\star$ goes to infinity while $f$ vanishes. This is weird. But it happens because, on the one hand, the factor looks like a U-shaped function in the context defined by $\pi$, and, on the other hand, the loss incurred in regions where the factor vanihes, $\ell(0,g)=g$, is only linear in~$g$, so it's ``mild'' even for large values of~$g$ when weighted by~$\pi$. This is consistent with the fact that the KL~divergence is an inclusive  measure (``zero avoiding'' for $g_\star$). In contrast, minimizing $\pi(f-g)^2$ would yield a well-behaved (bell-shaped) factor approximation -- to see this, just realize that $\inf_g\int\pi(f-g)^2 \leq \int\pi f^2$, which is finite.  

\subsection{Sufficient conditions for VS convergence}

In summary, sufficient conditions for $\sigma_0^2$ to be finite are:
\begin{enumerate}
\item The factor $f$ is upper bounded.
\item The ``posterior'' variance is smaller than twice the ``prior'' variance.
\end{enumerate}

It is easy to see that these conditions imply that each element of $\Sigma$ is finite, so they are sufficient for VS convergence. The key reason is that $\pi g_\star^2$ is a Gaussian distribution if condition~2 holds (as opposed to a U-shaped log-quadratic function), so $\int \pi g_\star^2 \phi \phi^\top$ is finite. 

The condition on the posterior variance feels mild because VS is meant to process Bayesian posteriors that may be sharply peaked around their mode. But such situation might be questionable in very large dimension, where the variance may be flat in some directions. Besides, it is difficult to test beforehand (the posterior variance is what we aim to estimate).

As already pointed out, truncating the reference distribution wouldn't really solve the problem. The asymptotic variance would become finite, but still possibly way too big. In the counter-example, how would we know beforehand that we should truncate outside $[-a,a]$?
...
Note that the same story applies to $f\phi$ instead of just $f$ to deal with moments of order-1, order-2, and so on.


\subsection{Asymptotic variance}

Quite clearly, the condition that $\|f\|_\infty$ is finite is sufficient to guarantee that $\var[f(X)]$ is finite, and similarly for $\var[f(X)\phi(X)]$ for all $\phi$, which guarantees in turn the convergence of IS in the context of sampling from a Gaussian distribution $\pi$.
...
For VS, however, we have an extra condition on $g_\star$. Intuitively, since $g_\star$ is the best fit to $f$ in the KL~sense, the residuals $f-g_\star$ tend to be smaller than wrt a constant baseline, so we expect $\int \pi(f-g_\star)^2$ to be smaller than $\var[f(X)]$. We could try and make this a little bit more precise using the following argument:
$$
\min_g \int \pi (f-g)^2 \leq \min_{c\in\mathbb{R}} \int \pi (f-c)^2 = \var[f(X)].
$$

However, $g_\star$ does not minimize $\pi (f-g)^2$, it minimizes the KL divergence $D(\pi f\|\pi g)$, therefore the above reasoning doesn't work, as shown by the above counter-example.

We learn several things from that.
\begin{enumerate}
\item VS may not converge while IS converges, even very slowly.
\item Therefore, VS is not necessarily better than IS.
\item The intuition that $g_\star$ follows $f$ closely is not always correct. 
\item This intuition may not be correct even though the factor is ``almost'' perfectly log-quadratic on the domain covered by $\pi$. In the counter-example, VS will actually be much more stable than IS in practice. Why? Just think about it: because on a finite sample drawn from $\pi$, chances are that we don't get any point outside $[-a,a]$, where $f$ is perfectly log-quadratic. 
\item The sample size required to reach asymptotic regime may heavily depend on the factor at hand. 
\end{enumerate}
...
Does this upper bound help undestanding when VS converges? 
...
At this point, we see that a sufficient condition for VS to converge is that both $f$ and $g_\star$ are upper bounded. This condition on $g_\star$ is equivalent to assuming that $g_\star$ is bell-shaped, that is, $\prec(g_\star)\succeq 0$. Because $\prec(\pi f)=\prec(\pi g_\star)=\prec(\pi) + \prec(g_\star)$, this boils down to the assumption:
$$
\prec(\pi f) \succeq \prec(\pi),
$$
or,
$$
\var(\pi f) \preceq \var(\pi)
.
$$

This condition is just more stringent than the one we identified before, without being easier to verify, therefore it's not super useful. But it does tell us the variance inequality yields sufficient conditions for convergence that are quite similar to the direct analysis, so the inequality gives some useful insight into what is going on.
...
$$
\delta_\alpha (f, g) \equiv
D(\pi f^\alpha \|\pi g^\alpha),
$$
where $\alpha$ is a positive scaling factor. The intuition behind this generalization is that the scaled target factor $f^\alpha$ gets flatter as $\alpha$ approaches zero, hence the VS estimator should have lower variance considering that the approximating family contains constant functions. This assumes that the VS variance is continuous in~$\alpha$ but I can't see any reason to think otherwise. On the other hand, applying a scaling factor~$\alpha< 1$ amounts to ``shrinking'' the context with respect to the factor, hence biasing the target distribution  towards the particular context.

This is a typical bias/variance tradeoff. Could the Expectation Propagation (EP) solve it? More exactly, its ``averaged'' variant (AEP).

Consider rewriting the target distribution as follows:
$$
p = \pi \prod_{k=1}^n f^\alpha,
\quad
\alpha = \frac{1}{k}.
$$

Assume that we have a current approximation $g^\alpha$ for $f^\alpha$. Initially, this approximation could be just $g^\alpha\equiv 1$. 

The AEP update consists of fitting the factor $f^\alpha$ in the cavity $c=\pi g^{1-\alpha}$ by minimizing $D(cf^\alpha\|cg^\alpha)$ over a new $g$. This can be approximated using VS, with $c$ as the context, and $f^\alpha$ as the target factor. This is arguably a better conditioned problem than the original one in the sense that VS may converge faster.

We see that, in the first iteration, since $g^\alpha=1$, the AEP update boils down to the scaling approach we have just proposed before.

In the second iteration, the context would incorporate some information from the target distribution as the $g^\alpha$ output by VS would approach $f^\alpha$, therefore the new context would resemble a bit more $\pi f^{1-\alpha}$. By iterating this process, we might get a fix point for $g$ that is actually the minimizer of the $\alpha$-divergence $D_\alpha(\pi f\|\pi g)$. 

This could solve the chicken-and-egg problem I mentioned in my early work on VS: $\pi$ would be ideally chosen depending on a Gaussian approximation to the target distribution $p$, but we need $\pi$ to compute such an approximation via VS...

The drawback is that we are no longer minimizing the KL divergence $D(\pi f\|\pi g)$, but (hopefully) the $\alpha$-divergence $D_\alpha(\pi f\|\pi g)$. This might be the price to pay to work around slow VS convergence.
