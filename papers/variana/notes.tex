...
In the following, we consider the canonical problem of minimizing $D(p\|q)$ for some target~$p$, wrt some distribution log-quadratic distribution~$q$. I wrote the following section at Covii in Spring~2019. The goals is to have a scalable optimizer for VS, and the core idea is to re-parameterize the approximating family locally so as to implement~VS using a first-order gradient descent rather than a quasi-Newton method, which requires maninpulations on some Hessian approximation in one way or another and may thus be challenging in very large dimension. 
