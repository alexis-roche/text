...
\section{2023 thoughts}

Context: ill posedness (in the sense of Hadamard) of empirical risk minimization, including  maximum likelihood, MAP, etc. These methods are at the heart of statistical inference and machine learning, still they are based on ill-posed mathematical problems! Shouldn't we worry about that?

The empirical risk is just a proxy for a Bayesian risk that we are not able to compute unless we know the {\em true} distribution of the training data. We don't really mean to optimize the empirical risk.

Bayesian inference boils down to a problem of the form:
$$
r(\theta) = \int p(x) L(x, \theta) dx 
$$
where $p(x)$ is the distribution of some variable of interest and $\theta$ is the parameter to be learned. Because we don't know $p(x)$ in agnostic learning context, or we know it but it's intractable, we may resort to an empirical version of the risk:
$$
\hat{r}(\theta) = \frac{1}{n} L(x_i, \theta).
$$
